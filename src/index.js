...
module.exports = function (api) {
    ...
    api.chainWebpackMainElectronProcess((cfg) => {
        if(api.ctx.modeName == "electron") {
            ...
            cfg.resolve.alias.set("SecureWindow", path.resolve(__dirname, "lib", "secure-window"))
            // cfg.plugin("electron-security").use(webpack.DefinePlugin, [{
            //     ELECTRON_SECURITY: JSON.stringify(api.prompts)
            // }])
        }
    ...
    api.extendQuasarConf((cfg, api) => {
        if(api.ctx.modeName == "electron") {
            ...
            if(api.prompts.hasOwnProperty("inject-electron") && api.prompts["inject-electron"].includes(true)) {
                cfg.boot.push("~quasar-app-extension-electron-security/src/boot/inject-electron.js")
            }
        }
    })
}
